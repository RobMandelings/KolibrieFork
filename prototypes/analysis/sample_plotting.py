...
import statistics
from typing import Mapping, Literal, Optional, Iterable, Dict
...
def extract_samples_by_strategy(per_workload_strats: dict) -> dict:
    return {
        strategy: data["throughput"]["sample"]
        for strategy, data in per_workload_strats.items()
        if "throughput" in data and "sample" in data["throughput"]
    }


def plot_samples_grouped(
        per_workload_strats: dict,
    ...
    fig, ax = plt.subplots()
    samples = extract_samples_by_strategy(per_workload_strats)
